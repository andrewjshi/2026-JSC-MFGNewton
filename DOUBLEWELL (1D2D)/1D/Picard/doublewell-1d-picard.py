...
x  = xmin + (np.arange(Nx) + 0.5) * Dx  # Cell centers on the bounded interval
num_x = x.size
...
# Forward difference.  The missing outward difference at the right boundary is
# set to zero, consistently imposing the homogeneous Neumann HJB condition.
data_p = np.array([-np.ones(num_x), np.ones(num_x)])
D_plus = sp.spdiags(data_p, [0, 1], num_x, num_x).tolil()
D_plus[num_x-1, :] = 0.0
D_plus = D_plus.tocsr() / Dx

# Backward difference with the missing outward difference at the left boundary
# set to zero.
...
# so its transpose is also the conservative no-flux FP diffusion operator.
...
def solve_fp_forward(U_flow):
    ...
    for n in range(Nt):
        ...
        # Hamiltonian Jacobian.  Together with the Neumann Laplacian, this
        # gives a conservative no-flux density update.
...
with open(history_filename, "w", buffering=1) as f_log:
    header = (
        ...
        f"  tilt = {tilt}, well_depth = {well_depth}, initial_std = {initial_std}, boundary = homogeneous Neumann/no-flux\n"
        f"{'-'*118}\n"
