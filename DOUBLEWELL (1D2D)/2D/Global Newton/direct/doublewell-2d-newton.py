...
x   = xmin + (np.arange(Nx) + 0.5) * Dx  # Cell centers on the bounded domain
y   = ymin + (np.arange(Ny) + 0.5) * Dy  # Cell centers on the bounded domain
...
# Forward difference at interior faces.  The missing outward difference on
# the last row is zero, which imposes homogeneous Neumann data for the HJB.
data_px = np.array([-np.ones(Nx), np.ones(Nx)])
D_plus_1Dx = sp.spdiags(data_px, [0, 1], Nx, Nx).tolil()
D_plus_1Dx[Nx-1, :] = 0.0
D_plus_1Dx = D_plus_1Dx.tocsr() / Dx

# Backward difference at interior faces.  The missing outward difference on
# the first row is zero, again imposing homogeneous Neumann HJB data.
data_mx = np.array([-np.ones(Nx), np.ones(Nx)])
D_minus_1Dx = sp.spdiags(data_mx, [-1, 0], Nx, Nx).tolil()
D_minus_1Dx[0, :] = 0.0
D_minus_1Dx = D_minus_1Dx.tocsr() / Dx

# Corresponding bounded-domain differences in y.
...
# Conservative homogeneous-Neumann Laplacian.  This symmetric negative
# semidefinite operator has zero row and column sums, so its FP contribution
...
def global_newton_step(nu_val, W_guess, log_file=None):
    ...
    for it in range(NEWTON_MAX_ITER):
        ...
        for n in range(Nt):
            ...
            # discrete HJB--FP adjoint relation and zero boundary flux.
...
with open(history_filename, "w", buffering=1) as f:
    header = (
        ...
        f"  initial_std = {initial_std}, boundary = homogeneous Neumann/no-flux\n"
        f"{'-'*118}\n"
    )
