...
# Cell-centered bounded-domain grid.  The physical walls are at xmin/xmax and
# ymin/ymax; homogeneous Neumann conditions are imposed on the HJB equation,
# and the FP equation has the corresponding conservative no-flux condition.
...
# One-sided x differences with zero outward difference at the walls.  These
# encode the homogeneous Neumann HJB boundary condition without wrapping the
# nonperiodic double-well potential across the domain boundary.
data_p_x = np.array([-np.ones(Nx), np.ones(Nx)])
Dp_x_1d = sp.spdiags(data_p_x, [0, 1], Nx, Nx).tolil()
Dp_x_1d[-1, :] = 0.0
Dp_x_1d = Dp_x_1d.tocsr() / Dx

# Backward x difference, also zero at the outward wall.
...
# One-sided y differences with zero outward difference at the walls.
data_p_y = np.array([-np.ones(Ny), np.ones(Ny)])
Dp_y_1d = sp.spdiags(data_p_y, [0, 1], Ny, Ny).tolil()
Dp_y_1d[-1, :] = 0.0
Dp_y_1d = Dp_y_1d.tocsr() / Dy

# Backward y difference, also zero at the outward wall.
...
# Symmetric finite-volume Neumann Laplacian.  Its row and column sums vanish,
# so the same operator gives HJB Neumann diffusion and conservative FP
# diffusion.  Together with J_H.T below, this makes the full FP spatial
# operator the exact discrete adjoint of the linearized HJB spatial operator.
...
with open(history_filename, "w", buffering=1) as f_log:
    header = (
        ...
        f"  initial_std = {initial_std}, boundary = homogeneous Neumann HJB / no-flux FP\n"
        f"{'-'*118}\n"
