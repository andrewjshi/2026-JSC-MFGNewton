...
# --- Saved File Names ---
history_filename      = f"newton-type2-history_congestionlocal2d_T{T}_nu{nu}.txt"
plot_contour_filename = f"localcongestion2d-contour_T{T}_nu{nu}.png"
plot_surface_filename = f"localcongestion2d-surface_T{T}_nu{nu}.png"
...
def iterative_solve(A, b, x0=None):
    """BiCGSTAB with ILU preconditioner — used for the large 2D systems."""
    A_csc = A.tocsc()
...
def solve_fp_forward(U_flow, M_old_flow):
    ...
    for n in range(Nt):
        ...
        vx, vy   = -dH_dp(grad_ux, m_ref), -dH_dp(grad_uy, m_ref)
...
with open(history_filename, "w") as f_log:
    header = (
        f"{'='*118}\n"
        f"   LOCAL CONGESTION MFG EXAMPLE 2D (NEWTON TYPE 2: NEWTON-PICARD)\n"
        f"{'='*118}\n"
        f"Parameters:\n"
